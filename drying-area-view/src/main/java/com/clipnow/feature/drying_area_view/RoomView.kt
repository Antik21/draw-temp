package com.clipnow.feature.drying_area_view
...
import com.clipnow.feature.drying_area_view.renderer.data.RoomDataConsts
import com.clipnow.feature.drying_area_view.renderer.dpToPx
import com.clipnow.feature.drying_area_view.renderer.renderer.*
...
class RoomView @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        roomData?.let { data ->
            ...
            // Render windows
            data.perimeter.walls.flatMap { it.windows }.forEach { windowRenderer.render(canvas, it, ::toCanvasX, ::toCanvasY) }

            // Render openings
            data.perimeter.walls.flatMap { it.openings }.forEach { openingRenderer.render(canvas, it, ::toCanvasX, ::toCanvasY) }

            // Render doors
            data.perimeter.walls.flatMap { it.doors }.forEach { doorRenderer.render(canvas, it, ::toCanvasX, ::toCanvasY) }

            // Render length lines
            data.perimeter.walls.flatMap { it.lengthLines }.forEach { lengthLineRenderer.render(canvas, it, ::toCanvasX, ::toCanvasY) }
        }
